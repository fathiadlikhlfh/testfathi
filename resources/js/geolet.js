/*
GEOLET version 20.12.31 --- DEMO VERSION
Copyright (c) 2020 Ruben Holthuijsen; available under the MIT license
https://github.com/rhlt/leaflet-geolet/

THIS IS A DEMO VERSION WITH DEMONSTRATION CONTENT, PLEASE DO NOT HOTLINK THIS FILE DIRECTLY
INSTEAD, DOWNLOAD THE ORIGINAL FROM GITHUB (link above) AND UPLOAD IT TO YOUR WEB SERVER
*/
...
(function () {
  ...
  console.warn(_consolePrefix + "You're using a DEMO VERSION of Geolet.js. Download the original from GitHub: https://github.com/rhlt/leaflet-geolet/");
  ...
  var _defaultMarker = function (controlOptions) {
    ...
    return L.marker(null, {
      icon: L.divIcon({
        html: '<svg width="20" height="20" viewport="0 0 20 20"><circle class="user-location-circle" cx="10" cy="10" r="10" style="fill:red"/><circle cx="10" cy="10" r="7" style="fill:white;stroke:white;stroke-width:2px"/><circle cx="10" cy="10" r="4" style="fill:red"/></svg>',
        iconAnchor: [10, 10],
        className: "",
      }),
  ...
  var _generateSvgIcon = function (options) {
    if (typeof options != "object") options = {};
    var thumb = typeof options.thumb != "undefined" ? options.thumb : null;

    var svgIcon = '<svg width="30" height="30" viewport="0 0 30 30">';

    var fillArg = 'fill="#777" fill-opacity="0.7"';
    var strokeArg = 'stroke="#777" stroke-width="2"';
    ...
    if (thumb) {
      svgIcon +=
        ...
        '<image xlink:href="' +
        thumb +
        '" x="1" y="1" width="28" height="28" clip-path="circle(14px at 14.5px 14.5px)"></image>' +
        '<circle cx="15" cy="15" r="14" ' +
        strokeArg +
        ' fill-opacity="0" />';
    } else {
      svgIcon += '<circle cx="15" cy="15" r="14" ' + strokeArg + " " + fillArg + " />";
    }
  ...
  L.Geolet = L.Control.extend({
    ...
    initialize: function (options) {
      //Set control options
      if (typeof options == "object" && options !== null) L.setOptions(this, options);
    },
    ...
    activate: function () {
      ...
      var geoSuccessCallback = function (data) {
        ...
        if (control._popup) {
          if (control._popup.isOpen() && control.options.updatePopupWhenOpen) control.updatePopup();
        }
        if (control._marker) {
        ...
        if (control._first) {
          ...
          if (control.options.autoPan) control._map.setView(control._latLng, control.options.minZoom ? Math.max(control._map.getZoom(), control.options.minZoom) : control._map.getZoom());
        }
    ...
    styleAnchor: function (active) {
      ...
      if (active && this.options.activeClassName) className.push(this.options.activeClassName);
      if (className.length) {
        this._a.className = className.join(" ");
      } else {
        this._a.className = "";
      }
      if (active && this.options.activeStyle) _merge(this._a.style, this.options.activeStyle);
      if (!active && this.options.style) _merge(this._a.style, this.options.style);
    },
    ...
    onAdd: function (map) {
      ...
      if ("fetch" in window) {
        //Add a random marker for demonstration purposes

        fetch("https://www.rubenholthuijsen.nl/random-photo.php")
          ...
          .then(function (data) {
            ...
            var icon = L.divIcon({
              ...
              html: _generateSvgIcon({
                thumb: data.thumbnail,
              }),
              bgPos: [15, 15],
              iconAnchor: [15, 15],
            });
            var marker = L.marker([data.lat, data.lng], { icon: icon, title: "GEOLET DEMO VERSION -- Try clicking the location button at the bottom left of the screen" });
            marker.on("click", function () {
              window.open("https://www.flickr.com/photos/rubenholthuijsen/" + data.id);
            });
            marker.addTo(map);
          });
      }
